...
def acc_login(request):
    ...
    random_filename = "".join(random.sample(string.ascii_lowercase, 4))  # 随机生成四个小写字母
    random_code = verify_code.gene_code(verify_code_img_path, random_filename)
    cache.set(random_filename, random_code, 30)  # 设置缓存，官网有详解
    ...
    if request.method == "POST":
        ...
        if cache.get(_verify_code_key) == _verify_code:
            ...
            user = authenticate(username=username, password=password)  # authenticate   官方认证方法
            if user is not None:
                login(request, user)
                request.session.set_expiry(60 * 60)
                # 设置会话的到期时间。您可以传递许多不同的值：
                # 如果value是整数，则会话将在多秒不活动后到期。
                # 例如，调用 request.session.set_expiry(300)会使会话在5分钟后到期。
...
def enrollment(request, customer_id):
    ...
    model_form = king_admin_forms.create_form(models.Enrollment,
                                              fields,
                                              site.enabled_admins[models.Enrollment._meta.app_label][
                                                  models.Enrollment._meta.model_name])
            ...
            if exist_enrollment_objs.filter(contract_agreed=True):
                ...
                if enroll_obj.contract_approved or request.POST.get('contract_approved') == "on":
                    ...
                    if enroll_obj.paymentrecord_set.select_related().count() > 0:  # already has payment record
                        ...
                        response_msg = {'msg': 'contract approved, waiting for payment record to be created', 'code': 3,
                                        'step': 4}
                        ...
                        model_form = king_admin_forms.create_form(models.PaymentRecord,
                                                                  fields,
                                                                  site.enabled_admins[
                                                                      models.PaymentRecord._meta.db_table])
